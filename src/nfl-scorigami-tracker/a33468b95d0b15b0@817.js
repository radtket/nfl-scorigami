...
function _1(md) {
  return md`
<div style="color: grey; font: 13px/25.5px var(--sans-serif); text-transform: uppercase;"><h1 style="display: none;">Color legend</h1><a href="https://d3js.org/">D3</a> › <a href="/@d3/gallery">Gallery</a></div>
...
function _11(Legend, d3) {
  return Legend(
    d3.scaleThreshold([2.5, 3.1, 3.5, 3.9, 6, 7, 8, 9.5], d3.schemeRdBu[9]),
    {
      title: 'Unemployment rate (%)',
      tickSize: 0,
    }
...
function _14(Swatches, d3) {
  return Swatches(
    d3.scaleOrdinal(['blueberries', 'oranges', 'apples'], d3.schemeCategory10)
  );
}
...
function _legend(Legend) {
  return function legend({ color, ...options }) {
...
function _Swatches(d3, htl) {
  return function Swatches(
    ...
    if (format === undefined)
      format = x => {
        return x === unknown ? formatUnknown : x;
      };
...
.${id}::before {
  ...
  <div>${domain.map(value => {
    return htl.html`<span class="${id}" style="--color: ${color(value)}">${format(value)}</span>`;
  })}</div>`;
  };
}

function _swatches(Swatches) {
  return function swatches({ color, ...options }) {
...
export default function define(runtime, observer) {
  ...
  main
    ...
    .define('Swatches', ['d3', 'htl'], _Swatches);
  main
    .variable(observer('swatches'))
    .define('swatches', ['Swatches'], _swatches);
  return main;
}
